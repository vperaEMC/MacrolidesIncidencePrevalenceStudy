...
cdm_aco <- DrugUtilisation::generateDrugUtilisationCohortSet(
  cdm = cdm_aco,
  name = "prevalence chronic use aco",
...
cdm_aco <- IncidencePrevalence::generateDenominatorCohortSet(
  cdm = cdm_aco,
  name = "denominator_prevalence_chronic_use_aco",
  #cohortDateRange = as.Date(c("2010-01-01", "2022-12-31")),
...
prev_chronic_use_aco <- IncidencePrevalence::estimatePeriodPrevalence(
  cdm = cdm_aco,
  denominatorTable = "denominator_prevalence_chronic_use_aco",
  outcomeTable = "prevalence chronic use aco",
...
prev_chronic_use_aco_adj <-
  prev_chronic_use_aco %>%
  dplyr::mutate(
...
write.csv(prev_chronic_use_aco_adj, here::here("Macrolides/prev_chronic_use_aco_adj.csv"))