...
for(i in seq_along(macrolides)){
  ...
  if(macrolides[[i]]=='All macrolides'){
name_sec_x_axis <- 'Various time windows for continuous prescribing of any macrolide'
  } else {
    name_sec_x_axis <- paste0('Various time windows for continuous prescribing of ', 
                              tolower(macrolides[[i]]))
  }
...
name_sec_y_axis <- 'Databases'
  ...
  if(names(results_inc_prev)[k] == 'prevalence'){
    b2 <- 
      a22 + ggplot2::labs(x = "",
                         y = "Prevalence",
                         fill = fill_name,
                         title = paste0("Prevalence estimates with 95% confidence intervals of prescribed ",tolower(names(macrolides)[i]),
    ...
    b2 <- 
      a22 + ggplot2::labs(x = "",
                         ...
                         fill = fill_name,
                         title = paste0("Incidence estimates with 95% confidence intervals of prescribed ",tolower(names(macrolides)[i]),
                                        ", for various time periods, among study populations of interest, stratified by ", graph_topics[[j]])) +
      ggplot2::scale_y_continuous(
        #breaks=seq(0,0.8,0.05), 
  ...
  # add another label on top of existing facet labels

  # d2 +
  #   ggplot2::scale_y_continuous(sec.axis = sec_axis(~ . , name = "SECOND Y AXIS",
  #                                                   breaks = NULL, labels = NULL)) +
  # 
  #   ggplot2::scale_x_continuous(sec.axis = sec_axis(~ . , name = "SECOND X AXIS",
  #                                                   breaks = NULL, labels = NULL))
  
  
  # Add a grey rectangular bar with text above the plot
  
  # x = denominator_target_cohort_integer,
  #     y = point_estimate
  # d2 + 
  #   geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 300, ymax = 400), 
  #             fill = "grey80", alpha = 0.5) +  # Grey bar
  #   annotate("text", x = mean(data$x), y = max(data$y) + 1, 
  #            label = "Your Text Here", color = "black", size = 5)  # Text
                              
  # # Define custom labels
  # custom_labels <- c("A" = "Group A: Custom Label", "B" = "Group B: Custom Label")
  # 
  # # Apply custom labels using labeller
  # d2 + facet_wrap(~ group, labeller = labeller(group = custom_labels))
  ...
  #store all graphs in an object with some listy abracadabra
    ...
    names(last_graph) <- paste0(names(results_inc_prev)[k], '_', names(graph_topics)[j],'_',names(macrolides)[i])
